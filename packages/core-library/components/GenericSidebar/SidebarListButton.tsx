...
export const SidebarListButton = ({ navigation, pathname }: SidebarListButtonProps) => {
    ...
    return (
        <Box width="100%">
            ...
            {open && navigation.children ?
                navigation.children.length > 0 && navigation.children?.map((childNav, index) => (
                    <SidebarButton navigation={childNav} key={index} pathname={pathname} />
                )) : null
            }
            <Divider />
        </Box>
