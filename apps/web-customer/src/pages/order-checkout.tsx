...
import { LinkAuthenticationElement, useStripe, PaymentElement, useElements } from '@stripe/react-stripe-js'
...
const OrderCheckout: React.FC<Props> = (props) => {
    ...
    const updatePaymentIntent = useApiCallback(async (api, p: UpdatePaymentIntentParams) =>
    ...
    async function confirmPayment() {
        ...
        await updatePaymentIntent.execute({ ...obj })
        ...
        return (
            <FormProvider {...form}>
                <div className='w-full h-fit flex flex-col gap-2'>
                    ...
                    <button onClick={confirmPayment} className=' bg-gradient-to-b from-[#2253c3] to-[#6593ff] px-5 py-2 text-white font-semibold rounded-lg self-end mt-5'>Confirm Payment</button>
                </div>
            </FormProvider>
        )
    }
    ...
    return (
        <div className='w-screen h-screen flex'>
            <div className='py-5 px-10 bg-gradient-to-b from-[#2253c3] to-[#6593ff] h-full w-1/2'>
                <p className='w-full text-center pb-5 text-white font-semibold opacity-50'>NCLEX Power - Core Zigma System</p>
                ...
                <div className='p-5 text-white flex flex-col gap-5'>
                    ...
                    <p className='text-3xl px-5 font-semibold'>$ 260.00 <span className='text-[1.3rem]'>(USD)</span></p>
                </div>
            </div>
