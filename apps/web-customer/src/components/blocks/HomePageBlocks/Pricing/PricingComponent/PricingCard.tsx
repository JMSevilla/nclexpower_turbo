    ...
    <div
      onClick={() =>
        handleSelectProduct({
          ...
          inclusions: cardData.inclusions, // 0 = 23 Days : 1 = 8 Days
        })
      }
      ...
      <div className="w-full flex gap-5 pb-5 pt-3 justify-center items-center px-5 border-b">
        <div className="w-1/2 text-center">
          <p className="text-4xl font-semibold">
            ...
            <span className="text-sm">{cardData.pricing.currency}</span>
          </p>
        </div>
      </div>
      <div className="w-full flex gap-5 justify-center items-center py-5">
        <div className="text-xs flex flex-col gap-2">
          {cardData.inclusions.features?.length > 0 &&
            cardData.inclusions.features.map((label, index) => (
