...
export const PreloadedGlobalsProvider: React.FC<React.PropsWithChildren<Ssr>> = ({
    children,
    data
}) => {
    const getPreloadedHeaders = hooks.useApi(
        async (api) => {
            const result = await api.preloadedGlobals.getPreloadedGlobalsHeader({
                LNum: data.slug[0], accountId: data.slug[1]
            });
            return result.data 
        },
        []
    ...
    return(
        ...
        >{children}</PreloadedGlobalsContext.Provider>
    )
}
