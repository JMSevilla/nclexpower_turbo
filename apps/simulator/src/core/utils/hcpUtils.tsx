...
export const groupedByConsecutiveIndex = (words: selectedWordType[]) => {
  const groupedByConsecutiveIndex: selectedWordType[][] = words
    .sort((a, b) => a.wordIndex - b.wordIndex)
    .reduce<selectedWordType[][]>((groups, current, index, array) => {
      if (current) {
        if (
          index === 0 ||
          (current && current.wordIndex !== (array[index - 1]?.wordIndex ?? -1) + 1)
        ) {
          groups.push([current])
        } else {
...
export const removeArrayDuplication = (item: selectedWordType[]) => {
  const removedDuplication = item
    .filter((ele, ind) => ind === item.findIndex((elem) => elem?.wordIndex === ele?.wordIndex))
    .sort((a, b) => {
      if (a && b) {
  ...
  const handleMouse = () => {
    ...
    if (highlightedValue) {
      setHighlightedWords((prevValues) => {
        ...
        if (prevValues) {
          const removeRehighlighted = prevValues
            .map((value) => {
              const filterByValues = highlightedValue
                .map((f) => {
          ...
          const filterCombinedValues = combinedValues.filter(
            (values) => !removeRehighlighted.includes(values.wordIndex)
          )
...
export const renderHighlightText = (values: string[], highlightedWords: selectedWordType[]) => {
  ...
  return values.map((word, index) => {
    ...
    return word.includes('<br/>') ? (
      <span key={index} dangerouslySetInnerHTML={theObj} />
    ) : (
      <span
        key={index}
