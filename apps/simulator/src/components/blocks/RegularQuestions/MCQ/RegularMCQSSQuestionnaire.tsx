      ...
      .then(async (res: any) => {
        if (res.data === 200) {
          ...
          router.push({
            pathname: '/next-item',
            query: {
              slug: ['B850483A-AC8D-4DAE-02C6-08DC5B07A84C', 'C002B561-66AF-46FC-A4D2-D282D42BD774', 'true'],
            },
          });
        }
      });
          ...
          <Paper sx={{ width: '70%' }}>
            <div className="h-full w-full p-4">
              {itemselection &&
                ...
                itemselection.map((item: datatypes.CalcItemSelectValues, idx) => (
                  <div key={item.qId}>
                    <div>
                      {contents &&
                        contents.answerUI.length > 0 &&
                        contents.answerUI.map((answerMap: datatypes.AnswerUIItem) => (
                          <React.Fragment>
