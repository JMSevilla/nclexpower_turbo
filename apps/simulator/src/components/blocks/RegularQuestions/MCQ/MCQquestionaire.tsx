...
import { SsrData, SsrMockQuestionaire } from "@/core/types/ssrData";
import React, { useState } from 'react';
import NearMeIcon from '@mui/icons-material/NearMe';
import { Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Paper, Button, Grid, Container, Radio  } from '@mui/material';


export const McqQuestion: React.FC<SsrData> = ({ questionaire, answer }) => {
    const [selectedOptions, setSelectedOptions] = useState(Array(answer.length).fill(null));
                                            ...
                                            <Paper elevation={3}>
                                                <TableContainer>
                                                    <Table>
                                                        ...
                                                        <TableBody>
                                                            {answerItem.rows.map((row: any, rowIndex: any) => (
                                                                <TableRow key={row.rowIndex}>
                                                                    <TableCell align="left" className='text-sm border border-[#D4D7DA] p-4 w-40'>{row.NRI}</TableCell>
                                                                    <TableCell align="center" className=' border border-[#D4D7DA]'>
                                                                        <Radio
                                                                            value={row.ID}
                                                                            checked={selectedOptions[rowIndex] === 0}
                                                                            onChange={() => handleOptionChange(rowIndex, 0)}
                                                                        />
                                                                    </TableCell>
                                                                    <TableCell align="center" className=' border border-[#D4D7DA]'>
                                                                        <Radio
                                                                            value={row.NID}
                                                                            checked={selectedOptions[rowIndex] === 1}
                                                                            onChange={() => handleOptionChange(rowIndex, 1)}
                                                                        />
                                                                    </TableCell>
                                                                </TableRow>
                                                            ))}
