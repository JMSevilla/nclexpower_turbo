...
export const HCPHighlighter: React.FC<HCPHighlighterProps> = ({ textToHighlight, highlightedTexts }) => {
...
export function Highlighter<T extends FieldValues>({ control, name, content }: HighlighterProps<T>) {
  return (
    <Controller
      name={name}
      control={control}
      render={({ field: { onChange }, fieldState: { error } }) => {
        return <div>{content ? <HCPHighlighter textToHighlight={content} highlightedTexts={onChange} /> : null}</div>;
      }}
    />
  );
}
