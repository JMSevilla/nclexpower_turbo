...
import SATATable from '@/components/block/CaseStudy/SATA/SATATable'
...
export const SATAQuestion: React.FC<Props> = ({ question, answer, obj }) => {
            ...
            <div className='w-1/2 h-full px-10 py-5'>
                ...
                <div className='w-full h-full'>
                    {question && question.map((questionItem, questionIndex) => (
                        <div key={questionItem?.id} className='flex gap-1'>
                    ...
                    <Paper variant='elevation' className='rounded-b-md rounded-r-md h-5/6 max-h-[500px] p-5 overflow-y-auto flex flex-col gap-5'>
                        {question && question.map((questionItem) => (
                            <div key={questionItem?.qId} className='flex flex-col gap-y-4'>
