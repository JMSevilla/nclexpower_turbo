    ...
    <div className='w-50% h-fit flex flex-col justify-center items-center gap-2'>
      ...
      <div className='flex w-full gap-2'>
        <div className='w-1/2 flex flex-col gap-2'>
          <AnimatedBoxSkeleton borderRadius={2} boxShadow={1} height={60} className='opacity-50' />
          <AnimatedBoxSkeleton borderRadius={2} boxShadow={1} height={280} className='opacity-50' />
        </div>
        <div className='w-1/2 flex flex-col gap-2'>
          <AnimatedBoxSkeleton borderRadius={2} boxShadow={1} height={60} className='opacity-50' />
          <AnimatedBoxSkeleton borderRadius={2} boxShadow={1} height={280} className='opacity-50' />
...
export const ParseContents: React.FC<Props> = ({
  ...
  if (questionType === "RegularQuestion") {
    // If RegularQuestion is available, Show SkeletonLoader
    if (isLoading) {
      return <RegularQuestionSkeleton />
    } else {
    ...
    // If Case Study Question is available, Show SkeletonLoader
    if (isLoading) {
